...
public class Exam {
	...
	private String[] questionTypes = new String[] { "mcsaquestion", "mcmaquestion", "saquestion", "numquestion" }; // Array
																													// to
																													// hold
																													// question
																													// types.
	...
	public Exam(Scanner scn) {
		...
		while (scn.hasNextLine()) { // Reads until EOF.
			...
			while (fileLine.length() == 0) { // If the current line is empty (length 0) then skip to the next until it
												// is not.
				fileLine = scn.nextLine();
			}
			...
			for (int i = 0; i < questionTypes.length; i++) {
				...
				if ((fileLine.toLowerCase().equals(questionTypes[i]))) { // If the line equals one of the question
																			// types, then create the question.
	...
	public double getValue() {
		...
		if (studentScore == 0.0 && maxScore == 0.0) { // Error checking: in case there are no questions on the exam to
														// grade.
			examScore = -1.0;
		} else {
	...
	public void reportQuestionValues() {
		...
		for (Question q : questions) { // Goes through each question to print out relevant scoring information.
			System.out.println("+----           Question " + (i + 1) + " | "
					+ new DecimalFormat("#.##").format(q.getValue()) + "                    ----+"); // Keeps it to two
																										// decimal
																										// places.
			i++;
		}
